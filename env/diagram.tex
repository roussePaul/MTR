...
\draw [arrow] (quad.east) -- ++(1,0) -- ++(0,2) node[align=center,pos=0.5, anchor=east] {$x,\dot{x}$} -- (speed_controller);

\draw [arrow] (quad.east) -- ++(2,0)  -- ++(0,4) node[align=center,pos=0.75, anchor=east] {$v$} -- (controller);
