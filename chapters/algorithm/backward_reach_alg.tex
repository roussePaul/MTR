...
\KwResult{$\Fsc$}
 ...
 \While{$Q \neq \emptyset$}{
  ...
  $T_{escape}(F) < +\infty$
    ...
    Add $F$ to $K$\;
	Update visited set $K_{visited}$ with $K$\;	
	\uIf{$init \subseteq \tilde{K}$ and $G \subseteq Reach_{K}(init)$}{
       Add plan $K$ to $\mathcal{K}^\star$\;
     }
     \uElseIf{$K \notin K_{visited}$}
     {
       Add $List\{ (K,F) \mid m \in PrevModules(K)\} $ to $Q$\;
     }
   }
 }
