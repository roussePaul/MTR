...
\comment{Explain that this is only for the dynamical systems, other systems can be obtained by composition of systems. The escape might be also an assumption of the system (we know that the camera needs to have 5 seconds max of movie).}
...
\section{Conclusion}
\comment{How efficient is this model? It is the worst case scenario of the state (a rough upper bound that might be not).}

\comment{can we use other escape time property?}

\comment{Say something about MITL?}
pouet